...
const M: Record<string, Partial<Record<Locale, string>>> = {
  ...
  "dl.sub": { en: "Every platform, available now — Windows, macOS, Linux, Android, and iOS (sideload).", es: "Todas las plataformas disponibles — Windows, macOS, Linux, Android e iOS (sideload).", fr: "Toutes les plateformes — Windows, macOS, Linux, Android et iOS (sideload).", de: "Alle Plattformen verfügbar — Windows, macOS, Linux, Android und iOS (Sideload).", pt: "Todas as plataformas — Windows, macOS, Linux, Android e iOS (sideload).", zh: "全平台可用 — Windows、macOS、Linux、Android 和 iOS（侧载）。", ja: "全プラットフォーム対応 — Windows・macOS・Linux・Android・iOS（サイドロード）。", hi: "सभी प्लेटफ़ॉर्म उपलब्ध — Windows, macOS, Linux, Android और iOS (साइडलोड)।", ar: "كل المنصات متاحة الآن — Windows وmacOS وLinux وAndroid وiOS (تحميل جانبي)." },
  "dl.iosNote": { en: "iOS is an unofficial sideload build: Apple requires signing, so install the .ipa with a free Apple ID using", es: "iOS es una build no oficial (sideload): instala el .ipa con un Apple ID gratuito usando", fr: "iOS est une build non officielle (sideload) : installez le .ipa avec un identifiant Apple gratuit via", de: "iOS ist ein inoffizieller Sideload-Build: installiere die .ipa mit einer kostenlosen Apple-ID über", pt: "iOS é uma build não oficial (sideload): instale o .ipa com um Apple ID gratuito usando", zh: "iOS 为非官方侧载版本：使用免费 Apple ID 通过以下工具安装 .ipa：", ja: "iOSは非公式サイドロード版：無料のApple IDで.ipaをインストール：", hi: "iOS अनौपचारिक साइडलोड बिल्ड है: मुफ़्त Apple ID से .ipa इंस्टॉल करें:", ar: "iOS إصدار غير رسمي (تحميل جانبي): ثبّت ملف .ipa بمعرف Apple مجاني عبر" },
